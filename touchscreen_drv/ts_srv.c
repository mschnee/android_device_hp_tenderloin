...
int calc_point(void)
{
    ...
    int clc=0;
    struct candidate clist[get_config(MAX_CLIST)];
    ...
    for(i=0; i < 30; i++) {
        for(j=0; j < 40; j++) {
            ...
            if(matrix[i][j] > get_config(TOUCH_THRESHOLD) && clc < get_config(MAX_CLIST)) {
                int cvalid=1;
...
if(get_config(DEBUG))
    ...
    for(k=0; k < MIN(clc, 20); k++) {
        ...
        if(newtp && tpc < get_config(MAX_TOUCH)) {
            tweight=0;
            isum=0;
            jsum=0;
...
if(get_config(DEBUG))
    ...
    * re-touch */
    if (previoustpc == 1 && tpc == 1) {
        ...
        if (total_delta > get_config(MAX_DELTA))
            liftoff();
    }
